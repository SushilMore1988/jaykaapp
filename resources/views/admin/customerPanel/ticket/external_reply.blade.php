...
{{-- Select2  --}}
  ...
  <link rel="stylesheet" type="text/css" href="{{ asset('dist/css/custom-badges.css') }}">
  <link rel="stylesheet" type="text/css" href="{{ asset('dist/css/invoice-style.min.css') }}">
  <link rel="stylesheet" type="text/css" href="{{asset('dist/css/ticket-reply.min.css')}}">
  <link rel="stylesheet" type="text/css" href="{{ asset('dist/css/external-ticket.min.css') }}">
@endsection
@section('content')
  <div class="col-sm-12 p-4" id="cus-panel-ticket-reply-container">
    <div class="card">
      <div class="card-header">
        <h5 class="top-bar-title font-weight-bold p-0">{{ __('Ticket No') }}: #{{ $ticketDetails->id }}</h5>
        <span class="cbadge cbadge-outlined f-14 mb-1 border-B0BEC5" style="color: {{ isset($ticketDetails->ticketStatus->color) ? $ticketDetails->ticketStatus->color : '' }}" id="status_label">{{ isset($ticketDetails->ticketStatus->name) ? $ticketDetails->ticketStatus->name : '' }}</span>
      </div>
      <div class="card-block">
        ...
        <div class="row">
          <div class="col-md-12 col-sm-12 m-t-5">
            ...
            <span class="departmentText"> {{ isset($ticketDetails->department->name) ? $ticketDetails->department->name : '' }}</span></p>
            <?php
            ...
            <p class="cbadge cbadge-default cbadge-outlined mb-1">{{ __('Priority') }}:
              <span style="color: {{ $color }}"> {{ isset($ticketDetails->priority->name) ? $ticketDetails->priority->name : '' }}</span></p>
      ...
      <hr class="m-t-0">
      <div class="card-block">
        <form class="form-horizontal" action="{{ url('customer-ticket/externalReplyStore') }}" method="post" enctype="multipart/form-data" id="reply_form">
                ...
                <div class="card-body mx-0">
                  <div class="row">
                    <div class="col-sm-2 px-2">
                      <h6 class="media-heading txt-primary text-left">{{ isset($data->customer->name) ? $data->customer->name : '' }}</h6>
                          <img class="rounded-circle" width="45" height="45" alt=" " src='{{ url("dist/img/customer-avatar.jpg") }}'>
                      </a>
                    </div>
                    <div class="col-sm-10">
                      {!! $data->message !!}
                   ...
                   <div class="card admin-card">
                    <div class="card-body mx-0">
                      <div class="row d-flex flex-xl-row-reverse flex-md-row-reverse flex-sm-row-reverse">
                        <div class="col-sm-2 px-2">
                          <h6 class="media-heading txt-primary text-right">{{ isset($data->user->full_name) ? $data->user->full_name : '' }}</h6>
...
@section('js')
<script src="{{ asset('dist/js/html5lightbox/html5lightbox.js?v=1.0') }}"></script>
...
<script src="{{ asset('dist/js/custom/customerpanel.min.js') }}"></script>
@endsection
