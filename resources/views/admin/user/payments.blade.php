...
@section('content')
  <!-- Main content -->
  <div class="col-sm-12" id="user-inv-payment-container">
    <div class="card">
      <div class="card-header">
        <h5><a href="{{ url('users') }}">{{ __('Users') }}</a> >> {{ __('Invoice Payment') }} >> {{$user->full_name}} </h5>
          ...
          <div class="col-md-12 col-sm-12 col-xs-12 m-l-10">
            <div class="row mt-3 mb-2">
              ...
              <div class="col-md-3 col-sm-3 col-xs-11 mb-2">
                <select class="form-control valdation_select select2" name="customer" id="customer" required>
                  <option value="all" <?= ($customer=='all') ? 'selected' : ''?>>{{ __('All Customer') }}</option>
      ...
      <div class="card-block">
        <div class="col-sm-12">
          <div class="table-responsive">
            {!! $dataTable->table(['class' => 'table table-striped table-hover dt-responsive customer-payment', 'width' => '100%', 'cellspacing' => '0']) !!}
          </div>
        </div>
      </div>
    </div>
