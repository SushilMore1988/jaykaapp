...
<div class="col-sm-12" id="edit-ticket-container">
    <div class="card user-list">
        <div class="card-header">
            <h5><a href="{{ url('ticket/list') }}">{{ __('Tickets') }}</a> >> {{ __('Edit Ticket') }} #{{  $ticketDetails->id  }}</h5>
        ...
        <div class="card-block">
            ...
            <div class="tab-content pl-4" id="myTabContent">
                <div class="tab-pane fade show active" id="home" role="tabpanel" aria-labelledby="home-tab">
                    <form class="form-horizontal" action="{{url('ticket/update')}}" id="edit-ticket-form" method="post" enctype="multipart/form-data">
                        {{csrf_field()}}
                        <input type="hidden" name="menu" value="{{$menu}}">
                        <input type="hidden" name="sub_menu" value="{{ isset($sub_menu) ? $sub_menu : '' }}">
                        <input type="hidden" name="ticket_key" value="{{ $ticketDetails->ticket_key }}">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-8">
                                        <input type="text" class="form-control" name="subject" value="{{ $ticketDetails->subject }}">
                                    </div>
                                </div>
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-8">
                                        <select name="project_id" class="form-control select2">
                                            ...
                                            @foreach($projects as $data)
                                                <option value="{{$data->id}}" <?= $data->id == $ticketDetails->project_id ? 'selected' : ''?>>{{$data->name}}</option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-3">
                                        <select name="assign_id" class="form-control select2">
                                            ...
                                            <option <?= $data->id == $ticketDetails->assigned_member_id ? 'selected' : ''?> value="{{$data->id}}">{{$data->full_name}}</option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="col-sm-3">
                                        <select name="status_id" class="form-control select2">
                                            @foreach($ticketStatus as $data)
                                                <option <?= $data->id == $ticketDetails->ticket_status_id ? 'selected':'' ?>  value="{{ $data->id }}">{{ $data->name }}</option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="form-group row">
                                    <label class="col-sm-2 control-label require">{{ __('Customer') }}</label>
                                    <div class="col-sm-3">
                                        <select id="customer_id" name="customer_id" class="form-control select2">
                                            <option value="">{{ __('Select One') }}</option>
                                            @foreach($customers as $data)
                                                <option  <?= $data->id == $ticketDetails->customer_id ? 'selected' : '' ?> data-name="{{$data->first_name.' '.$data->last_name}}" data-email="{{$data->email}}"
                                                        value="{{$data->id}} ">{{$data->first_name.' '.$data->last_name}}</option>
                                            @endforeach
                                        </select>
...
<script src="{{ asset('dist/js/custom/ticket.min.js') }}"></script>
@endsection