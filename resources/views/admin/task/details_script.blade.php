   ...
   'use strict';
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          $.each(data.comment, function(key, value){
            ...
            comments += 
              '<div class="card task-card task_border" panel_id="'+value.id+'" >'+
                '<div class="p-1 card-header comment-card-header">'+ value.user_name +
                  '<span class="ml-50">'+
                ...
                '<div class="card-body comment-card-body comment_color">'+'<span class="content_area" id="content_area_'+value.id+'"  content="'+value.id+'">'+value.content+'</span>'+
                  '<span class="comment_textarea display_none" text_area_id ="'+value.id+'" >'+
                    ...
                    '<div class="form-group">'+
                      '<button class="mt-10 btn btn-danger btn-close-update custom-btn-small task-custom-btn-small" comment_id="'+value.id+'">{{ __('Cancel') }}</button>'+
                      '<button type="button" comment_task_id="'+value.task_id+'" comment_id="'+value.id+'"  class="mt-10 btn btn-primary custom-btn-small task-custom-btn-small pull-right btn-update-comment">{{ __('Update') }}</button>'+
          ...
          $.each(data.return_arr.checklist_items, function (key, value) {
            ...
            checklists += '<div class="m-0 mb-2 mr-3 form-group" id="checklist_row_'+value.id+'">'+
            ...
            if (status == 'checked') {
              checklists += '<span class="line-through break-all checklist_label" title="{{ __('Click to edit') }}"  data-id="'+value.id+'" id="checklist_label_'+value.id+'">'+ value.title +'</span>';
            } else {
              checklists += '<span class="break-all checklist_label" title="{{ __('Click to edit') }}" data-id="'+value.id+'" id="checklist_label_'+value.id+'">'+ value.title +'</span>';
            }
            checklists += '<i class="ml-2 feather icon-trash-2 cursor_pointer text-danger clickable delete-checklist-item col-form-label" data-id="'+value.id+'"></i></div>';
          ...
          if (data.status == 1) {
            ...
            $.each(data.oldAssignees, function(key, value) {
              ...
              if (value.picture != null) {
                assigneePic = '<div class="mr-2 task-modal-assignee-first-circle"><a target="_blank" href="{{URL::to("user/team-member-profile")}}/'+value.user_id+'">'+
                '<img alt="User profile picture" src="'+SITE_URL+'/uploads/user/'+ value.picture +'" class="mx-1 user-img" title='+ value.user_name +'>'
              +'</a></div>'+
              '</div>';
              } else {
                var userName = '';
                ...
                assigneePic = '<div class="mr-2 task-modal-assignee-first-circle"><a target="_blank" href="{{URL::to("user/team-member-profile")}}/'+value.user_id+'">'+
                '<span class="mx-1 alpha f-12" title='+ value.user_name +'>'+ userName +'</span>'
                +'</a></div>'+
                '</div>';
              }
              assignee_members +='<div class="float-left assigned" span_id = "'+value.user_id+'">'+ 
    ...
    $(document).on('click', '#add-checklist', function () {
      if ($(".add-checklist-item").length == 0) {
        var newChekclist =  '<div class="mx-0 form-group row add-checklist-item">'+
                              '<div class="col-sm-11">'+
        ...
        $.ajax({
          ...
          success: function (data) {
            if (data.message == "success") {
              var value = data.item;
              var item = '<div class="m-0 mb-2 mr-3 form-group" id="checklist_row_'+value.id+'">'+
                            '<div class="checkbox checkbox-primary d-inline">'+
                            ...
                            '<i class="ml-2 feather icon-trash-2 cursor_pointer text-danger clickable delete-checklist-item col-form-label" data-id="'+value.id+'"></i>'+
                          '</div>';
        ...
        success:function(data){
          if (data.old_member_status == 1) {
            ...
            $.each(data.old_members, function(key, value){
              var pic = value.picture ?  'uploads/user/'+ value.picture : 'dist/img/avatar.jpg'; 
                assignee_members +='<div class="m-0 row" span_id = "'+value.user_id+'">'+ 

                '<a class="col-md-2" target="_blank" href="{{URL::to("user/team-member-profile")}}/'+value.user_id+'">'+
                  '<img alt="User profile picture" src="'+SITE_URL+'/'+pic+'" class="m-2 user-img" width="30" height="30" title='+ value.user_name +'>'
                +'</a>'+
    ...
    $('#assignee_dropdown').on('change',function() {
      ...
      $.ajax({
        ...
        success:function(data){
          if (data.status == 1) {
            var assignee_members = '';
            var pic = data.assinee_details.picture ?  'uploads/user/'+ data.assinee_details.picture : 'dist/img/avatar.jpg';

            var assigneePic = "";
            if (data.assinee_details.picture != null) {
              assigneePic = '<div class="mr-2 task-modal-assignee-first-circle"><a target="_blank" href="{{URL::to("user/team-member-profile")}}/'+data.assinee_details.id+'">'+
              '<img alt="User profile picture" src="'+SITE_URL+'/uploads/user/'+ data.assinee_details.picture +'" class="mx-1 user-img" title='+ data.assinee_details.full_name +'>'
              +'</a></div>'+
              '</div>';
            } else {
              var userName = '';
              ...
              userName = userName.substring(0, 2);
              assigneePic = '<div class="mr-2 task-modal-assignee-first-circle"><a target="_blank" href="{{URL::to("user/team-member-profile")}}/'+data.assinee_details.id+'">'+
              '<span class="mx-1 alpha f-12" title='+ data.assinee_details.full_name +'>'+ userName +'</span>'
              +'</a></div>'+
              '</div>';
            }
            ...
            assignee_members = '';
            var pic = data.assinee_details.picture ?  'uploads/user/'+ data.assinee_details.picture : 'dist/img/avatar.jpg'; 
            assignee_members +='<div class="m-0 row" span_id = "'+data.assinee_details.id+'">'+ 

              '<a class="col-md-2" target="_blank" href="{{URL::to("user/team-member-profile")}}/'+data.assinee_details.id+'">'+
                '<img alt="User profile picture" src="'+SITE_URL+'/'+pic+'" class="m-2 user-img" width="30" height="30" title='+ data.assinee_details.full_name +'>'
              +'</a>'+
    ...
    function addComment(data) {
      $('#btn_comment').attr("disabled", false);
            ...
            var comments = '<div class="card task-card task-card-border" panel_id="'+data.comment_id+'" >'+
                '<div class="p-1 card-header comment-card-header">'+ data.user_name +
                '<span class="ml-50">'+
                '<i class="feather icon-clock ">'+
                '</i>'+data.created_at+
                '</span>'+
                ...
                '</textarea>'+
                 '<div class="form-group">'+
                    '<button class="mt-10 btn btn-danger btn-sm btn-close-update" comment_id="'+data.comment_id+'">{{ __('Cancel') }}</button>'+
                    '<button type="button" comment_task_id="'+data.task_id+'" comment_id="'+data.comment_id+'"  class="mt-10 btn btn-primary custom-btn-small task-custom-btn-small pull-right btn-update-comment">{{ __('Update') }}</button>'+
        ...
        success:function(data){  
          if (data.status == 'success') {
            ...
            if (jQuery.inArray(data.file_type, image_ext)!='-1') {
              var pic = 'uploads/task_files/'+ data.file_name;
            ...
            var download_path = 'uploads/task_files/'+ data.file_name;
