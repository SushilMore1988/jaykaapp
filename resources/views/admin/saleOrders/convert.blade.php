...
@section('content')
<div id="sales-quotation-convert-invoice-container">
          ...
          <div class="row">
            <div class="col-md-6">
              <div class="form-group row">
                ...
                <div class="col-md-8 p-md-0">
                  <label class="col-form-label">
                    {{ isset($invoiceData->customer->first_name) ? $invoiceData->customer->first_name : "" }} {{  isset($invoiceData->customer->last_name) ? $invoiceData->customer->last_name : "" }} ({{ $invoiceData->currency->name }})
                    ...
                    @endif
                    @if( isset($invoiceData->customerBranch->country))
                      , {{ $invoiceData->customerBranch->country->name }}
                    @endif
                  </label>
              ...
              <div class="form-group row">
                ...
                <input type="hidden" name="location_id" id="location" value="{{ isset($invoiceData->location->id) ? $invoiceData->location->id : '' }}">
              </div>
              <div class="form-group row" id="exchangeRateBlock">
                ...
                <div class="inv-exchange-rate col-md-8 p-md-0">
                  <input name="exchange_rate" id="inv-exchange-rate" class="form-control positive-float-number" value="{{ $invoiceData->exchange_rate }}" >
                </div>
              </div>
            </div>
            <div class="col-md-6">
          ...
          <div class="row">
            <div class="col-sm-8 mb-3">
              <div class="form-group row mb-0">
                ...
                <input class="form-control auto col-md-9" placeholder="Search Item" id="search">
                      ...
                      @foreach ($invoiceData->saleOrderDetails as $result)
                        ...
                        <tbody id="rowId-{{$count}}">
                          <tr class="itemRow rowNo-{{$count}}" id="{{'itemId-' . $result->item_id}}"  data-row-no="{{$count}}">
                            ...
                            <input type="hidden" name="old_item_details_id[]" value="{{ $result->id }}">
                            <input type="hidden" name="old_hidden_price[]" value="{{ $result->item_price }}">
                            <td class="pl-1">
                              <input name="old_item_name[]" id="item_name_{{$count}}" value="{{ $result->item_name }}" placeholder="Item Name" type="text" class="inputDescription form-control">
                            </td>
                            <td class="itemHSN">
                            ...
                            <td class="itemPrice">
                              <input  value="{{ formatCurrencyAmount($result->unit_price) }}" name="old_item_price[]" id="item_price_{{$count}}" class="inputPrice form-control text-center positive-float-number" type="text" placeholder="0.00">
                            </td>
                            <td class="itemDiscount">
                              <input name="old_item_discount[]" value="{{ formatCurrencyAmount($result->discount) }}" class="inputDiscount form-control text-center positive-float-number" id="item_discount_{{$count}}" type="text" placeholder="0.00">
                            </td>
                            <td class="itemDiscount">
                  ...
                  <div id="itemInputFooter" class="table-responsive">
                    <table>
                      ...
                      @foreach(json_decode($taxes) as $tax)
                        <tr class="taxRow" id="taxRow-{{ $tax->id }}">
                          ...
                          <td colspan="2">
                            ...
                            <input value="{{ $tax->tax_rate }}" name="tax" class="form-control display_none" id="inputTaxPercentage-{{ $tax->id }}" type="text">
                          </td>
                          <td ><span id="taxTotalValue-{{ $tax->id }}">0</span></td>
                        </tr>
                      @endforeach
                      <tr class="itemTaxExtra">
                      ...
                      <tr class="otherDiscount">
                        <th class="text-right">{{ __('Other Discounts') }}</th>
                        <td>
                          <input class="form-control text-right" value="{{ formatCurrencyAmount( $invoiceData->other_discount_amount) }}" name="other_discount_amount" class="form-control positive-float-number" placeholder="0.00" id="inputOtherDiscount" type="text">
                        </td>
                        <td class="text-right">
                        ...
                        <td><span class="currencySymbol"></span><span id="otherDiscountTotal">0</span></td>
                      </tr>
                      <tr class="shippingAmount">
                        ...
                        <td ><span id="shippingTotal">{{ !empty($invoiceData->shipping_charge) ? formatCurrencyAmount($invoiceData->shipping_charge) : formatCurrencyAmount(0) }}</span></td>
                      </tr>
                      
                      <tr class="customAmount">
                        <th>
                          <input value="{{ $invoiceData->custom_charge_title }}" name="custom_charge_title" class="form-control text-right positive-float-number" type="text" id="customAmountDescription" placeholder="Custom Amount">
                        </th>
                        <td colspan="2">
                          <input value="{{ $invoiceData->custom_charge_amount }}" name="custom_charge_amount" class="form-control text-right positive-float-number" placeholder="0.00" id="inputCustomAmount" type="text">
                        </td>
                        <td><span id="customAmountTotal">0</span></td>
                      </tr>
                      <tr class="grandTotal">
      ...
      <div class="card-body">
        <div class="m-t-10 p-0">
            ...
            <div class="col-md-11">
              <div class="dropzone-attachments" id="reply-attachment">
                <div class="event-attachments">
                  @forelse($files->files as $key => $file)
          ...
          <div class="form-group row ml-1">
            ...
            <a href="{{url('/order/view-order-details/'.$invoiceData->id)}}" class="btn btn-danger custom-btn-small float-left">{{ __('Cancel') }}</a>
          </div>
        </div>
      </div>
    </div>
...
<script src="{{ asset('dist/js/custom/invoice-script.min.js') }}"></script>
...
<script src="{{ asset('dist/js/custom/sales-purchase.min.js') }}"></script>
