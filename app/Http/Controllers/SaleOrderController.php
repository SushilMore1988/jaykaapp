...
class SaleOrderController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $url = DB::transaction(function () use ($request, $userId) {
            ...
            if (!empty($request->attachments)) {
                $path = createDirectory("uploads/invoice_order");
    ...
    public function edit($orderNo)
    {
        ...
        if (!empty($data['files'])) {
            $data['filePath'] = "uploads/invoice_order";
    ...
    public function copyOrder($orderNo)
    {
        try {
            ...
            if ( count($saleOrder->saleOrderDetails) > 0 ) {
                ...
                $files = (new File)->copyFiles("uploads/invoice_order", "uploads/invoice_order", $saleOrder->order_type, $orderNo, "Direct Order", $newOrder->id, ['isOriginalNameRequired' => true]);
            }
    ...
    */
    public function autoInvoiceCreate($orderNo)
    {
        ...
        $data['files'] = (new File)->copyFiles("uploads/invoice_order", "contents/temp", $data['invoiceData']->order_type, $orderNo, "Indirect Invoice", null, ['isTemporary' => true]);
        $data['files'] = json_decode($data['files']);
        $data['filePath'] = "contents/temp";
        $data['locations']      = Location::getAll()->where('is_active', 1);
        $data['currencies']     = Currency::getAll();
        $data['paymentTerms']   = PaymentTerm::getAll();
        $data['taxTypeList']    = TaxType::getAll();
