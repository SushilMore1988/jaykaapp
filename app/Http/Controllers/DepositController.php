    ...
    public function store(Request $request)
    {
       ...
       try {
          ...
          if (!empty($request->attachment) && !empty($deposit_id)) {
            $path = createDirectory("uploads/deposit");
      ...
      try {
          DB::beginTransaction();
            ...
            if (!empty($request->attachment) && !empty($request->id)) {
                $data = (new File)->deleteFiles('DEPOSIT', $request->id, [], 'uploads/deposit');
                $path = createDirectory("uploads/deposit");
    ...
    public function destroy($id)
    {
      ...
      if (!empty($id)) {
        try {
            ...
            if ($deposit->delete()) {
              (new File)->deleteFiles('DEPOSIT', $id, [], 'uploads/deposit');
