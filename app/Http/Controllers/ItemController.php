...
class ItemController extends Controller
{
        ...
        try {
            ...
            if (isset($pic) && !empty($pic)) {
                if (isset($id) && !empty($id)) {
                    $path = createDirectory("uploads/items");
                    ...
                    if (isset($fileIdList[0]) && !empty($fileIdList[0])) {
                        ...
                        $thumbnailPath = createDirectory("uploads/items/thumbnail");
    ...
    public function updateItemInfo(Request $request)
    {
        ...
        if (isset($pic) && ! empty($pic)) {
            ...
            if (!empty($fileIds)) {
                (new File)->deleteFiles('Item', $request->id, ['ids' => [$fileIds], 'isExceptId' => false], $path = 'uploads/items');
            }
            # end region
