...
class TaskController extends Controller
{
            ...
            # region store files
                if (isset($insertId) && !empty($insertId)) {
                    if (!empty($request->attachments)) {
                        $path = createDirectory("uploads/tasks");
    ...
    public function taskDelete(Request $request)
    {
        try {
            ...
            if (isset($task_id)) {
                ...
                if ($record) {
                    $deleteTaskFiles = (new File)->deleteFiles('Task', $request->task_id, [], $path = 'uploads/tasks');
    ...
    public function uploadEventAttachments(Request $request, $save = false)
    {
        ...
        if (!empty($files)) {
            $user_id = Auth::user()->id;
            $fileDirectory = createDirectory("uploads/tasks");
    ...
    {
        $id = $request->id;
        return (new File)->deleteFiles("Task", $request->task_id, ['ids' => [$id]], "uploads/tasks");
    }

    public function downloadAttachment($id)
    {
        $file = DB::table('transaction_files')->where('id', '=', $id)->first();
        $dir_path = "uploads/transactionFiles";
