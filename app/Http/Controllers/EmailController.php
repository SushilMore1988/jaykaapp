...

{
            ...
            if (!empty($attachments)) {
                ...
                if (!empty($files)) {
                    foreach ($files as $file) {
                        if (!empty($file->file_name) && file_exists('uploads/tasks/'. $file->file_name)) {
                            $relatedFiles[] = url('uploads/tasks/'. $file->file_name);
                        }
                    }
                }
    ...
    public function sendTicketEmail($to, $subject, $messageBody, $data, $type = null, $attachments = null)
    {
        ...
        if (!empty($attachments)) {
            ...
            if (!empty($files)) {
                foreach ($files as $file) {
                    if (!empty($file->file_name) && file_exists('uploads/tickets/'. $file->file_name)) {
                        $relatedFiles[] = url('uploads/tickets/'. $file->file_name);
                    }
                }
            }
        }
    ...
    public function sendEmailWithAttachment($to, $subject, $messageBody, $invoiceName, $companyName = null)
    {
        ...
        $dataMail = array(
            ...
            'subject' => $subject,
            'content' => $messageBody,
            'attach'  => url("uploads/invoices/$invoiceName"),
        );
