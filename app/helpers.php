...
{
    $path = createDirectory("contents/temp");
    $mpdf = new \Mpdf\Mpdf([
        'tempDir' => $path,
        'mode'        => 'utf-8',
        'format'      => 'A4',
...
function translateValidationMessages()
{
    ...
    if (!empty($flag) && file_exists(public_path('/lang/validation/'. $flag .'.js'))) {
    // if (!empty($flag) && file_exists(public_path('../resources/lang/validation/'. $flag .'.js'))) {
        return '<script src="' . asset('lang/validation/'. $flag .'.js') . '"></script>';
        ...
        if (!empty($userId)) {
            ...
            foreach ($userId as $value) {
                if (isset($userPic[$value]) && file_exists(public_path('uploads/user/'. $userPic[$value]))) {
                    $image[$value] =  url('uploads/user/'. $userPic[$value]);
                } else {
                    $image[$value] = url("dist/img/avatar.jpg");
                }
            }
        }
